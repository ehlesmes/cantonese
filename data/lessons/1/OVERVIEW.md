# Chapter 1: Pronunciation (Jyutping)

Welcome to the first chapter of your Cantonese journey! This chapter establishes your foundation in
**Jyutping** pronunciation.

## Lesson 1.1: Upper Register Tones

- **Concepts:** High Level (1), High Rising (2), Mid Level (3).
- **Examples:**
  - `saam1` (三) - Three
  - `hou2` (好) - Good
  - `sei3` (四) - Four
- **Exercises:**
  - `1.1.2` (Reading): Differentiation between Tone 1, 2, 3.
  - `1.1.3` (Unscramble): Tone order challenge.

## Lesson 1.2: Lower Register Tones & Nasals

- **Concepts:** Low Falling (4), Low Rising (5), Low Level (6), and Syllabic Nasals (`m`, `ng`).
- **Examples:**
  - `jan4` (人) - Person
  - `nei5` (你) - You
  - `si6` (是) - Yes
  - `m4` (唔) - Not
  - `ng5` (五) - Five
- **Exercises:**
  - `1.2.2` (Reading): Lower tone recognition (`你是人`).
  - `1.2.3` (Unscramble): Lower tone phrase construction (`你是人`).
  - `1.2.4` (Reading): Syllabic nasals (`唔五人`).
  - `1.2.5` (Unscramble): Syllabic nasal phrase (`唔五人`).

## Lesson 1.3: Initials & Aspiration

- **Concepts:** Aspiration (puff of air) distinction for B/P, D/T, and G/K.
- **Examples:**
  - **B/P:** `baat3` (八) / `paak3` (拍)
  - **D/T:** `daai6` (大) / `taai3` (太)
  - **G/K:** `gaa1` (加) / `kaa1` (卡)
- **Exercises:**
  - `1.3.2` (Reading): B vs P differentiation (`八拍`).
  - `1.3.3` (Unscramble): B vs P challenge.
  - `1.3.4` (Reading): D vs T differentiation (`太大`).
  - `1.3.5` (Unscramble): D vs T challenge.
  - `1.3.6` (Reading): G vs K differentiation (`加各卡`).
  - `1.3.7` (Unscramble): G vs K challenge.

## Lesson 1.4: Basic Vowels

- **Concepts:** Simple vowels: `aa`, `i`, `u`, `e`, `o`.
- **Examples:**
  - `faa1` (花) - Flower (`aa`)
  - `si1` (詩) - Poem (`i`)
  - `ji5` (以) - Take (`i`)
  - `fu3` (褲) - Pants (`u`)
  - `de6` (地) - Ground (`e`)
  - `ho2` (可) - Can (`o`)
- **Exercises:**
  - `1.4.2` (Reading): Simple vowel recognition (`花詩以`).
  - `1.4.3` (Unscramble): Combining basic vowels (`詩人`).
  - `1.4.4` (Reading): Simple vowel recognition (`褲地可`).
  - `1.4.5` (Unscramble): Combining basic vowels (`可以`).

## Lesson 1.5: Unique Vowels & Diphthongs

- **Concepts:** Complex vowels (`oe`, `eo`, `yu`) and Diphthongs (`ai`, `au`, `ei`, `oi`).
- **Examples:**
  - `hoeng1` (香) - Fragrant (`oe`)
  - `gong2` (港) - Port (`oe`)
  - `syu1` (書) - Book (`yu`)
  - `maai5` (買) - Buy (`ai`)
  - `maau1` (貓) - Cat (`au`)
  - `nei5` (你) - You (`ei`)
  - `hoi1` (開) - Open (`oi`)
  - `sam1` (心) - Heart (`am`)
- **Exercises:**
  - `1.5.2` (Reading): Recognizing complex vowels (`香港書`).
  - `1.5.3` (Unscramble): Unique vowel combination (`香港`).
  - `1.5.4` (Reading): Recognizing diphthongs (`買貓`).
  - `1.5.5` (Reading): Diphthong combination (`買書`).
  - `1.5.6` (Unscramble): Diphthong sentence (`你開心`).

## Lesson 1.6: Ending Consonants

- **Concepts:** Nasal endings (`-m`, `-n`, `-ng`) and Stop endings (`-p`, `-t`, `-k`).
- **Examples:**
  - **Nasals:** `saam1` (三), `san1` (新), `sang1` (生)
  - **Stops:** `jap6` (入), `baat3` (八), `luk6` (六)
- **Exercises:**
  - `1.6.2` (Reading): Nasal and Stop differentiation (`三生新`).
  - `1.6.3` (Unscramble): Final consonant review.
