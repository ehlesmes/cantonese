...
`;
  ...
  test("Extraction - Inline semantic units", () => {
    ...
    const unit0 = units[0];
    const unit1 = units[1];
    if (!unit0 || !unit1) throw new Error("Expected units to be defined");
    expect(unit0.characters).toBe("你好");
    expect(unit0.jyutping).toBe("nei5hou2");
    expect(unit0.translation).toBe("hello");
    expect(unit1.characters).toBe("唔該");
    expect(unit1.jyutping).toBe("m4goi1");
    expect(unit1.translation).toBe("excuse me");
  });
  ...
  test("Extraction - Block semantic units", () => {
    ...
    expect(u1.jyutping).toBe("ngo5");
    expect(u2.characters).toBe("想");
    expect(u2.jyutping).toBe("soeng2");
  });
